...
public class DrawReciept {
    ...
    private void printBottom(Order order){
        try {
            ...
            receivedBuffer.append(df.format(order.getOredrUserPayment()));
            ...
            double balance = 0.0;
            if ((order.getOredrUserPayment() > order.getOrderTotal())) {
                balance = order.getOredrUserPayment() - order.getOrderTotal();
            }
